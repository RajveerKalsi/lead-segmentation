...
const OUTPUT_ASIN_CSV = path.join(__dirname, "data", "brand_existance.csv");
...
async function processSingleBrand(browser, brand, maxCards) {
  ...
  try {
    ...
    if (results.length > 0) {
      for (let r of results) {
        const fetchedBrand = await fetchBrandFromASINPage(page, r.link);
        r.fetchedBrandName = fetchedBrand;
      }
      ...
      appendASINResults([
        {
          ...
          fetchedBrandName: "no valid product",
    ...
    console.error(`❌ Error processing brand "${brand}":`, err.message);
    await page.close();
  }
}
...
//     title: r.title || "",
//     fetchedBrandName: r.fetchedBrandName || "",
//     link: r.link || "",
...
function appendASINResults(results) {
  ...
  const cleanedResults = results.map((r) => {
    const sanitizedOriginal = sanitizeBrandName(r.brandName || "");
    const sanitizedFetched = sanitizeBrandName(r.fetchedBrandName || "");
    const brandExist = sanitizedOriginal.toLowerCase() === sanitizedFetched.toLowerCase() ? "Yes" : "No";
...
async function fetchBrandFromASINPage(page, asinLink) {
  try {
    await page.goto(asinLink, {
      waitUntil: "domcontentloaded",
      timeout: 60000,
    });

    const brandName = await page.evaluate(() => {
      const brandRow = document.querySelector("tr.po-brand span.po-break-word");
      return brandRow ? brandRow.textContent.trim() : null;
    });

    return brandName || "Not Found";
  } catch (err) {
    console.warn("⚠️ Failed to fetch brand from ASIN page:", err.message);
    return "Not Found";
  }
}
...
(async () => {
  ...
  await page.close(); // no longer needed

  const CONCURRENCY_LIMIT = 3;

  await runInBatches(brands, CONCURRENCY_LIMIT, (brand) =>
    processSingleBrand(browser, brand, maxCards)
  );
